...
Routes:
    ...
    GET  /admin/targets      → targets admin page (HTML)
    POST /admin/targets      → save targets (JSON API)
    GET  /admin/competitors  → competitors admin page (HTML)
    POST /admin/competitors  → save competitors (JSON API)
"""
...
async def targets_page(month: str | None = None):
...
@router.post("/targets")
async def save_targets(body: SaveTargetsBody):
...
async def competitors_page():
    ...
    store_options = "".join(f'<option value="{s}">{s}</option>' for s in STORES)
...
async def save_competitors(body: SaveCompetitorsBody):
    if not is_db_available():
        return JSONResponse({"ok": False, "error": "DATABASE_URL not set"}, status_code=503)
    try:
