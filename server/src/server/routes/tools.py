"""Admin Tools — super-admin-only file storage via MinIO.

Routes:
    GET  /admin/tools              → File manager page (HTML)
    POST /admin/tools/upload       → Upload file to MinIO
    GET  /admin/tools/files        → List files (JSON)
    GET  /admin/tools/files/{key}  → Download/stream file (proxied through server)
    DELETE /admin/tools/files/{key} → Delete file
"""
...
BUCKET = "haidilao-uploads"


def _get_minio_client():
    """Return a configured MinIO client, or raise HTTPException if not configured."""
    try:
...
<style>
  * { box-sizing: border-box; }
  ...
  .drop-zone { border: 2px dashed #ccc; border-radius: 8px; padding: 40px 20px;
  ...
  .copy-btn { cursor: pointer; background: #eee; border: none; border-radius: 3px;
              padding: 2px 8px; font-size: 0.78rem; margin-left: 8px; }
  .copy-btn:hover { background: #ddd; }
  .progress-bar { display: none; height: 6px; background: #eee; border-radius: 3px; margin-top: 12px; overflow: hidden; }
  .progress-bar-fill { height: 100%; background: #c0392b; width: 0%; transition: width 0.3s; }
  #msg { font-size: 0.88rem; margin-top: 10px; min-height: 1.2em; }
  .ok { color: #1a7a1a; }
  .err { color: #c0392b; }
  .size { color: #888; }
...
<header>
  ...
  <span class="user-info">👤 {user_name} &nbsp;·&nbsp; <a href="/admin/logout" style="color:rgba(255,255,255,0.75);font-size:0.82rem">退出</a></span>
</header>
<div class="container">
  <div class="card">
    <h2 style="margin-top:0">文件上传</h2>
    <p style="color:#555;font-size:0.9rem;margin-top:0">
      上传文件后，复制链接粘贴到 GitHub Issue，Agent 可通过本地端口读取。
      支持所有格式（图片、zip、Excel、PDF 等）。
    </p>
    <div class="drop-zone" id="drop-zone" onclick="document.getElementById('file-input').click()">
      <p>🗂 拖拽文件到此处，或点击选择文件</p>
      <p class="hint">所有格式均支持</p>
    </div>
  ...
  <div class="card">
    <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:16px">
      ...
      <button class="btn btn-primary" onclick="loadFiles()" style="font-size:0.82rem;padding:5px 12px">刷新</button>
    </div>
    <div id="file-list">
...
async function loadFiles() {{
  ...
  try {{
    ...
    let html = '<table><thead><tr><th>文件名</th><th>大小</th><th>上传时间</th><th>链接</th><th>操作</th></tr></thead><tbody>';
    for (const f of data) {{
      const url = '/admin/tools/files/' + encodeURIComponent(f.key);
      html += `<tr>
        <td><a class="file-link" href="${{url}}" target="_blank">${{f.name}}</a></td>
        <td class="size">${{formatSize(f.size)}}</td>
        <td class="size">${{formatDate(f.last_modified)}}</td>
        <td>
          <code style="font-size:0.78rem;background:#f5f5f5;padding:2px 6px;border-radius:3px">${{url}}</code>
          <button class="copy-btn" onclick="copyUrl('${{url}}')">复制</button>
        </td>
        <td><button class="btn btn-danger" onclick="deleteFile('${{f.key}}')">删除</button></td>
      </tr>`;
    }}
    html += '</tbody></table>';
...
function copyUrl(url) {{
  ...
  navigator.clipboard.writeText(full).then(() => showMsg('✓ 链接已复制', true));
}}
