...
def _notify_run(run: Run) -> None:
    ...
    if run.command == "competitor-takeout-report" and run.status.value == "success":
        try:
            from server.notify import notify_competitor_takeout_report_file
            report_path = _find_competitor_takeout_report_from_run(run)
            if report_path:
                notify_competitor_takeout_report_file(
                    report_path,
                    target_chat=run.notify_chat or "production_accounting_report_chat",
                )
        except Exception:
    ...
    if run.command == "zfi0049-report" and run.status.value == "success":
        try:
            from server.notify import notify_zfi0049_file
            report_path = _find_zfi0049_report_from_run(run)
            if report_path:
                notify_zfi0049_file(
                    report_path,
                    target_chat=run.notify_chat or "production_accounting_report_chat",
                    company_code=str(run.params.get("company_code", "")),
                    fiscal_year=int(run.params.get("fiscal_year", 0)),
                    posting_period=int(run.params.get("posting_period", 0)),
                )
        except Exception:
            pass

    if run.command == "gross-margin-report" and run.status.value == "success":
        try:
            from server.notify import notify_gross_margin_file
            report_path = _find_gross_margin_report_from_run(run)
            if report_path:
                notify_gross_margin_file(
                    report_path,
                    target_chat=run.notify_chat or "production_accounting_report_chat",
                    company_code=str(run.params.get("company_code", "")),
                    fiscal_year=int(run.params.get("fiscal_year", 0)),
...
def _find_zfi0049_report_from_run(run: "Run") -> "Path | None":
    """Locate the ZFI0049 export file from run logs."""
    import re
    from pathlib import Path
...
def _find_competitor_takeout_report_from_run(run: "Run") -> "Path | None":
    """Locate the competitor takeout report file from run logs."""
    import re
    from pathlib import Path
    from server.config import settings
    ...
    report_dir = settings.output_dir.resolve() / "daily-report" / "competitor-takeout"
    candidates = sorted(report_dir.glob("*.xlsx"), key=lambda p: p.stat().st_mtime, reverse=True)
    return candidates[0] if candidates else None


def _find_gross_margin_report_from_run(run: "Run") -> "Path | None":
    """Locate the gross margin report file from run logs."""
    import re
    from pathlib import Path

    if run.logs:
        m = re.search(r"Report saved to (.+gross_margin_.+\.xlsx)", run.logs, re.IGNORECASE)
        if m:
            p = Path(m.group(1).strip())
            if p.exists():
                return p
