...
def create_issue(
    ...
    row = db.fetchone(
        """
        INSERT INTO issues (title, description, type, priority, created_by)
        VALUES (%s, %s, %s, %s, %s)
        RETURNING *
        """,
        (title, description, type_, priority, created_by),
    )
    return row


def get_issues(status: str | None = None) -> list[dict]:
    """Return issues, optionally filtered by status. Most recent first."""
    db = get_db()
    if db is None:
        return []
    if status:
        return db.fetchall(
            "SELECT * FROM issues WHERE status = %s ORDER BY created_at DESC",
            (status,),
        )
    return db.fetchall("SELECT * FROM issues ORDER BY created_at DESC")
...
def get_issue(issue_id: int) -> dict | None:
    ...
    if db is None:
        return None
    return db.fetchone("SELECT * FROM issues WHERE id = %s", (issue_id,))


def update_issue(issue_id: int, **fields) -> dict | None:
    """Update allowed fields on an issue."""
    db = get_db()
    if db is None:
        return None
    allowed = {"title", "description", "type", "priority", "status", "assignee"}
    ...
    db.execute(
        f"UPDATE issues SET {set_clause}, updated_at = NOW() WHERE id = %s",
        tuple(params),
    )
    return get_issue(issue_id)
...
def add_issue_message(
    issue_id: int, sender: str, content: str, role: str = "user"
) -> dict | None:
    ...
    row = db.fetchone(
        """
        INSERT INTO issue_messages (issue_id, sender, role, content)
        VALUES (%s, %s, %s, %s)
        RETURNING *
        """,
        (issue_id, sender, role, content),
    )
    # Also touch updated_at on the parent issue
    db.execute("UPDATE issues SET updated_at = NOW() WHERE id = %s", (issue_id,))
    return row


def get_issue_messages(issue_id: int) -> list[dict]:
    """Return all messages for an issue, oldest first."""
    db = get_db()
    if db is None:
        return []
    return db.fetchall(
        "SELECT * FROM issue_messages WHERE issue_id = %s ORDER BY created_at ASC",
        (issue_id,),
    )


def get_active_issues() -> list[dict]:
    """Return issues that need agent attention (open, planning, approved, in_progress)."""
    db = get_db()
    if db is None:
        return []
    return db.fetchall(
        """
        SELECT * FROM issues
        WHERE status IN ('open', 'planning', 'approved', 'in_progress')
        ORDER BY
            CASE priority
                WHEN 'urgent' THEN 0
                WHEN 'high'   THEN 1
