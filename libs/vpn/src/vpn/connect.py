"""SealSuite VPN connection middleware.

Call ``ensure_vpn()`` before any automation that requires corporate network
access.  It will:

1. Launch SealSuite if it is not running.
2. Turn VPN on if it is off.
3. If VPN is already on but the session has been connected for more than
   ``max_connected_hours`` (default 6), cycle the connection to avoid a
   mid-automation timeout (session expires after 7 h 30 min).
"""
...
from vpn.errors import VPNAppNotFoundError, VPNConnectionError
