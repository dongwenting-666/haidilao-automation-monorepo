"""E2E test: full ZFI0156 store-consumption-report flow (connect → login → export → disconnect).
...
Usage:
    uv run --project libs/sap-gui python libs/sap-gui/tests/e2e_zfi0156.py
"""
...
OUTPUT_DIR = Path(__file__).resolve().parents[3] / "output" / "zfi0156"
...
def main() -> None:
    ...
    log.info("Starting ZFI0156 E2E test")
    log.info("  User:    %s", username)
    log.info("  Plants:  %s - %s", DEFAULT_PLANT_LOW, DEFAULT_PLANT_HIGH)
    ...
    result = run(
        ...
        plant_low=DEFAULT_PLANT_LOW,
        plant_high=DEFAULT_PLANT_HIGH,
        date_from=d_from,
        date_to=d_to,
        language=language,
    )
