...
│   ├── llm.py         # LLM-based analysis (kept for future use, not imported)
...
uv run --project projects/ksb1-accounting-check python -m ksb1_accounting_check.main --output-dir ./my-output
```
...
3. **Enrich rows** — Add `月份` (from 过账日期) and `科目` (from mapping) to each row
...
All analysis is **deterministic** — no LLM, no randomness. Same input always produces the same output.
